...
def get_project_student_id(
    project_id: int, user_id: int, *, db: SqlRunner
) -> int | None:
    return (
        db.query("""
...
def insert_pending_submission(
    project_student_id: int, storage_path: str, content_hash: str, *, db: SqlRunner
) -> UUID:
    return (
        db.query("""
        ...
        RETURNING p_uuid
    """)
        .bind(ps_id=project_student_id, path=storage_path, hash=content_hash)
        .scalar(lambda x: x)
    )


def get_submission_by_uuid(
    submission_uuid: UUID, *, db: SqlRunner
) -> dict[str, Any] | None:
    return (
        db.query("""
