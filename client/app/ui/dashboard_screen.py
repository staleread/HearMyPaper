...
class DashboardScreen(QWidget):
    ...
    def init_ui(self):
        ...
        profile_btn.setStyleSheet(
            "background-color:#3F51B5; color:white; padding:12px; border-radius:8px; font-size:16px;"
        )
        profile_btn.clicked.connect(self.go_to_profile)
        layout.addWidget(profile_btn)

        submissions_btn = QPushButton("Go to Submissions")
        submissions_btn.setStyleSheet(
            "background-color:#2196F3; color:white; padding:12px; border-radius:8px; font-size:16px;"
        )
    ...
    def go_to_submissions(self):
        self.navigator.navigate_submissions(
            "Student", self.projects
        )  # або передати роль динамічно
